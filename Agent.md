...
* **顯示過濾規則 (Visibility)**：
  ...
  * **隱藏參照列 (Hide Reference Row)**：在 `node2.html` 等自由配置的樹狀圖中，允許透過獨立配置選項隱藏參照層的顯示。
...
## **6\. 圖片匯出技術限制與實作 (Export Constraints & Implementation)**

由於 `html2canvas` 在處理複雜 CSS 佈局與 `bottom` 座標時存在已知限制，AI 代理在修改匯出邏輯時必須注意：

*   **頂層透明背景強制化**：
    *   在呼叫 `html2canvas` 前，必須先將 `tree-container` 或 `canvas-bounds` 的 `style.background` 備份並暫時設為 `transparent`。
    *   即使傳入了 `{ backgroundColor: null }`，若 DOM 元素本身帶有背景色，輸出仍會帶顏色。必須在 `finally` 區塊恢復背景色。
*   **座標對齊補償 (Coordinate Alignment)**：
    *   `html2canvas` 對於運用 `calc()` 產生的 `absolute` 定位可能產生像素偏移。
    *   在 `rect-node` 系列中，匯出前需動態將所有 `bottom` 座標轉換為基於 `offsetTop` 的基準座標（Top-based），以確保畫面上看到的參照列位置與箭頭連線完整精確。
    *   **輸入框文字偏移**：匯出時會動態注入臨時 CSS `display: flex; align-items: center;` 至輸入框容器，解決 `input` 文字在畫布中垂直偏下的問題。
